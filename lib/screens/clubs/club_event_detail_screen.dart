...
class _ClubEventDetailScreenState extends State<ClubEventDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    final full = snap != null &&
        snap.seatLimit > 0 &&
        snap.registeredCount >= snap.seatLimit;
    if (!mounted) return;
    setState(() {
      _event = event;
      _snapshot = snap;
  ...
  Widget build(BuildContext context) {
    ...
    final hostName =
        event.clubName.trim().isEmpty ? 'ADA Clubs' : event.clubName;
    final hostInitial = hostName.characters.first.toUpperCase();
                        ...
                        ? () {
                            Navigator.push(
                              ...
                              MaterialPageRoute<void>(
                                builder: (_) =>
                                    EventTicketScreen(eventId: event.id),
                              ),
                            );
                          }
                        : (_full
                            ...
                            : () async {
                                try {
                                  ...
                                  final snap = await _repo
                                      .getEventSnapshot(event.id.toString());
